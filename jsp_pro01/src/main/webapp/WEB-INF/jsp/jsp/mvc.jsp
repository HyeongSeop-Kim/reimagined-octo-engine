...
<body>
	...
	<h2>Model 1</h2>
	<p>
		 고전적 웹 서비스 모델 방식으로 하나의 JSP에 화면 구성, 비즈니스 로직, 데이터베이스 연결 처리 등을 구성하여<br>
		 운용하는 형태의 모델 방식이다.
	</p>
	<p>
		개인이 서비스를 개발하는 경우에는 편하게 개발을 진행 할 수 있는 방식이다.<br>
		단, 시간이 흘러서 유지보수가 자주 이루어진 경우 이전에 작업한 코드의 분석이 어려워진다.<br>
		하나의 페이지에 HTML, CS, JS, JSP, Java, SQL 등의 코드가 섞여 있기 때문에 로직을 파악하는 것이 어렵다.<br>
		협업은 원활히 이루어 질 수 없다.
	</p>
	<hr>
	<h2>MVC Model(Model 2)</h2>
	<p>
		Model 1에서 분리가 안된 코드들을 기능별/역할별로 코드를 분리하여 관리하는 방식<br>
		새로운 기능/역할 추가가 용이해 진다.
	</p>
	<ul>
	...
	<ol>
		...
		<li>DeptController 에서는 dept.servic.Deptservice 를 사용하여 전체 부서를 조회하도록 한다.</li>
		<li>DeptService 에서는 dept.model.DeptDAO 를 사용하여 데이터베이스에 조회 요청을 하게 한다.</li>
		<li>DeptDAO 는 마이바티스 이용해서 데이터베이스 연결 세션을 생성 후 사용하게 된다.</li>
		...
		<li>모든 작업이 뫈료되어 완성된 HTML 코드를 클라이언트에게 응답 메시지로 전송한다.</li>
	</ol>
