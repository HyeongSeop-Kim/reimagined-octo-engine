...
public class Sample01 {
	...
	public static void main(String[] args) {
		 ...
		 *  	- 스트림을 통해 데이터를 입/출력 할 때 문자기반스트림과 바이트기반스트림으로 분류한다.
		 *  	- 문자기반스트림의 경우 일반 텍스트 문서를 입/출력하기 위해 사용(보통 txt 확장자를 가지는 파일)
		 *  	- 바이트기반스트림의 경우 일반 텍스트 문서를 제외한 모든 파일을 입/출력하기 위해 사용
		 *  	  (영상, 음향, 이미지, 각종 실행파일 등등...)
		 */
		
		File f = new File("C:/Users/KHS/eclipse/jee-2021-12/eclipse/eclipse.ini");
		
		System.out.printf("파일 크기 : %d Byted\n", f.length());
		System.out.printf("파일 크기 : %.2f KByted\n", f.length() / 1024.0);
		
		Date d = new Date(f.lastModified());
		SimpleDateFormat s = new SimpleDateFormat("yyyy년 MM월 dd일 a hh시 mm분");
		...
		File f2 = new File("C:/Users/KHS/eclipse/jee-2021-12/eclipse/my_file.txt");
		try {
			f2.createNewFile();
		} catch (IOException e) {
		...
		File f3 = new File("C:/Users/KHS/eclipse/jee-2021-12/eclipse/my_folder");
		f3.mkdir();
		
		File f4 = new File("C:/Users/KHS/eclipse/jee-2021-12/eclipse/my_folder");
		f4.delete();
		
		File f5 = new File("C:/Users/KHS/eclipse/jee-2021-12/eclipse");
